...
An IoT-based real-time LPG monitoring system that continuously measures cylinder weight, detects gas level percentage, identifies potential leaks, and triggers automatic alerts.  
...
Additionally, sudden abnormal weight drops are detected to identify potential gas leaks. Real-time data is uploaded to the cloud via Wi-Fi, enabling remote monitoring.

---

## 🏗 System Architecture

Load Cell → HX711 Amplifier → ESP32 → WiFi → ThingSpeak Cloud → Alerts
...
✔ Gas leak detection (sudden weight drop logic)  
